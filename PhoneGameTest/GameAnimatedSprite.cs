        ...
        public override void Update(RenderContext renderContext)
        {
            if (IsPlaying && !IsPaused)
            {
                ...
                if (_totalFrameTime >= FrameInterval)
                {
                    ...
                    if (_rowCount > 1)
                    {
                        _frameRect.Location = new Point(FrameSize.X * (CurrentFrame % _columnCount),
                            FrameSize.Y * (int)Math.Floor(CurrentFrame / _columnCount));
                    }
                    else _frameRect.Location = new Point(FrameSize.X * CurrentFrame, 0);
                    DrawRect = _frameRect;
                    ++CurrentFrame;
